...
public:
...
#ifdef _DEBUG
    void SaveMatrix(wchar_t *fileName, Matrix<ElemType> m)
    {
    ...
    virtual void /*ComputationNodeNonLooping::*/ ForwardPropNonLooping() override // -sum(left_i * log(softmax_i(right)))
    {
        ...
        //SaveMatrix(L"D:\\temp\\LFMMI\\priorities\\p.txt", m_softmaxOfRight);
        ...
        //SaveMatrix(L"D:\\temp\\LFMMI\\priorities\\gamma.txt", m_posteriors);
#ifdef _DEBUG
        ...
        //SaveMatrix(L"D:\\temp\\LFMMI\\priorities\\label.txt", Input(0)->MaskedValueFor(fr));
        Value() *= (ElemType)-1.0;
        Value() += logfwscore;
#if NANCHECK
